...
final class ArchiverTest: XCTestCase {
    ...
    func testArchiveAndThenUnarchive() throws {
        throw XCTSkip("這個測試會體現出，encode 跟 decode 的邏輯不一致，看來 data model 也要測試一下。除了 data model，這邊的測試也是必要的，因為測試的對象不一樣")
        let dummyHistoricalRate = ResponseDataModel
            .HistoricalRate(historical: true,
                            date: .now,
